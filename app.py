...
try:
    ...
    with tab_ho:
        ...
        # Filtros e Interactividad (Código anterior mantenido)
    ...
    # ---------------------------------------------------------
    # PESTAÑA 2: ANÁLISIS DE TIEMPOS (NUEVA LÓGICA)
    # ---------------------------------------------------------
    with tab_tiempos:
        st.header("⏱️ Análisis de Tiempos (Lead Times)")
        
        # Columnas de fecha necesarias
        cols_fechas = [
            "Fecha de Facturacion", "Fecha que el Gestor Retira Doc", 
            "Fecha Disponibilidad Papeles", "Fecha de confirmacion de entrega"
        ]
        ...
        mt4.metric("Ciclo Total Prom.", f"{df_t['Demora Total'].mean():.1f} d")

        st.divider()
        
        # Gráfico comparativo de demora total por vendedor
        st.write("### Demora Total Promedio por Vendedor (Días)")
        if "Vendedor" in df_t.columns:
            v_time = df_t.groupby("Vendedor")["Demora Total"].mean().sort_values().reset_index()
            fig_t = px.bar(v_time, x="Vendedor", y="Demora Total", color="Demora Total", 
                           color_continuous_scale="RdYlGn_r", template="plotly_white")
            st.plotly_chart(fig_t, use_container_width=True)

        st.subheader("📋 Detalle de Días por Etapa")
